<script lang="ts">
	...
	let toolHeadings = [
		{ tool: null, heading: '通用快捷键' },
		{ tool: Tools.TERRAIN, heading: '地形工具' },
		{ tool: Tools.ICON, heading: '图标工具' },
		{ tool: Tools.PATH, heading: '路径工具' },
		{ tool: Tools.TEXT, heading: '文本工具' },
		{ tool: Tools.ERASER, heading: '橡皮擦工具' },
	];
...
<div id="maps-container">
	<div id="maps">
		{#each toolHeadings as toolHeading}
			<h2 style={toolHeading.heading == '通用快捷键' ? 'margin-top: 0;' : ''}>
				{toolHeading.heading}
			</h2>
				...
				<span class="shortcut-listing">
					...
					<p class="keycode">
						{#if shortcut.displayKeycode}
							<div class:highlighted={isKeyDown(shortcut.keycode, keysDown)}>
								{shortcut.displayKeycode}
							</div>
						{:else}
